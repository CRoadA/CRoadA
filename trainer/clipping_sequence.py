...
class ClippingBatchSequence(Sequence):
    ...
    def __init__(self, base_sequence: BatchSequence, clipping_size: int, input_grid_surplus: int):
        ...
        """
        self._dir = "./tmp/clipping_batches/clipping_batch_sequences/cache/clipping_batch_sequence_" + str(1) + "_batch_" # TODO - time.time()
        ...
        if not os.path.isfile(batch_dir):
            ...
            for base_batch_index in range(len(self._base_sequence)):
                for base_batch_item_index in range(self._base_sequence.batch_size()):
                    ...
                    if result >= index:
                        batches_dir = self._base_sequence[base_batch_index]
                        with open(batches_dir, "rb") as f:
            ...
            with open(batch_dir, "wb") as f:
                pickle.dump((batch_x, batch_y_is_street, batch_y_altitude), f)
        else:
            with open(batch_dir, "rb") as f:
                batch_x, batch_y_is_street, batch_y_altitude = pickle.load(f)

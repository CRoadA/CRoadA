...
class Rasterizer():
    ...
    def get_rasterize_roads(self, gdf_edges : gpd.GeoDataFrame, is_residential : bool, pixel_size : int = 1):
        ...
        if is_residential:
            gdf_edges = gdf_edges[gdf_edges["is_residential"]]
    ...
    def rasterize_segment_from_indexes(self, gdf_edges: gpd.GeoDataFrame, indexes : tuple[int, int], size_w : int, size_h : int, is_residential : bool, pixel_size : int = 1) -> np.array:
        min_x, min_y, max_x, max_y = gdf_edges.total_bounds
        if (max_x - min_x) / pixel_size < size_w:
            size_w = math.ceil((max_x - min_x) / pixel_size)

        segment_min_x = min_x + indexes[1] * size_w * pixel_size
        segment_max_x = segment_min_x + size_w * pixel_size
        if segment_max_x > max_x:
            segment_max_x = max_x
            size_w = math.ceil((segment_max_x - segment_min_x) / pixel_size)


        if (max_y - min_y) / pixel_size < size_h:
            size_h = math.ceil((max_y - min_y) / pixel_size)
        segment_max_y = max_y - indexes[0] * size_h * pixel_size
        segment_min_y = segment_max_y - size_h * pixel_size
        if segment_min_y < min_y:
            segment_min_y = min_y
            size_h = math.ceil((segment_max_y - segment_min_y) / pixel_size)
